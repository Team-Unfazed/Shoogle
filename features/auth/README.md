...
Sign-in, sign-up, password reset, session handling, Supabase client usage, RLS policy design, and the shared infrastructure the other four features depend on.

## Where this appears
`app/(auth)/*`, and the `SessionProvider` mounted in `app/_layout.tsx`.
...
- `SessionProvider.tsx` - the session shell consumed by `app/_layout.tsx`.
  It reads and subscribes to the Supabase session and nothing more. Extend it;
  do not replace its contract, because both route groups depend on it.
- `lib/supabase/client.ts` - returns `null` when unconfigured, by design.

## Watch out

- The service role key must never be imported into the app. It is deliberately
  absent from `lib/env`.
- OAuth redirects use the `shoogle://` scheme, declared in `app.config.ts`.
