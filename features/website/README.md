...
Generating, previewing and publishing a simple website for the business.

## Where this appears
Reached from the Business tab.
...
- The status machine is `not_started -> generating -> ready_for_review ->
  published`, plus `failed`. Show the real state.
- **No progress theatre** (product rule 10). If you cannot measure real
  progress, show an indeterminate indicator - never a fake percentage or a
  checklist that ticks itself off on a timer.
- `publishedUrl` stays `null` until the site is genuinely live.
