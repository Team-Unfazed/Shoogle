...
The Home surface: what Shoogle suggests the owner does next, recent activity, and cross-module summaries.
...
- Home is the most tempting place to invent numbers. Every metric must come
  from a real, connected provider or render as unavailable.
- Suggestions must be actionable and explain themselves - Shoogle is an
  operator, not a dashboard (product rule 1).
